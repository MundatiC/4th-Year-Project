...
var getUserSettings = function() {
    for (var key in userSettings) {
        if (key === "UserID" || isOb(userSettings[key])) {
            // Skip UserID or any other object-like structures if present
            continue;
        }
        var element = document.getElementById(key);
        if (element === null) {
            console.error('No element found for ID:', key); // Help diagnose missing elements
        } else {
            o.userSettings[key] = userSettings[key];
            changeSettings(element, 'notNext');
        }
    }
    console.log('Final user settings:', o.userSettings);
	console.log('Final user settings:', userSettings);
};
